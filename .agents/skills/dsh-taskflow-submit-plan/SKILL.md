...
- Planning and `REPLAN` use the configured planning model/effort (default `gpt-5.6-sol / max`). Read `/plugins/taskflow/status` when the effective scene must be verified.
