...
- With P8.6 defaults, automation is on and `repoRoot` is set, so planning may start automatically after submit. Observe `/plugins/taskflow/state` or `/plugins/taskflow/run` instead of issuing duplicate `plan` calls.
